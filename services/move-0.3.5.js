 ...
 * 2017/8/17  Remove callback use Promise.
 * */

/**
 * 定时线性移动
 * @param obj 需要移动的元素
 * @param attrs 需要改变的样式 键值对 的形式  eg.{left: 200, top: 300}
 * @param duration 定时时间
 * @param fx 渐动函数
 */
function lMove(obj, attrs, duration, fx) {
  return new Promise((resolve, reject) => {
    ...
    for (let attr in attrs) {
      j[attr] = {};
      if (['scrollTop'].includes(attr)) {
        j[attr].startS = obj[attr];
      } else {
    ...
    obj.timer = setInterval(function () {
      let kkkTime = new Date().getTime() - oldTime;
      if (kkkTime >= duration) {
        kkkTime = duration;
      }
      for (let attr in j) {
        let v = Tween[fx](kkkTime, j[attr].startS, j[attr].move, duration);
        if (attr === 'opacity') {
          obj.style[attr] = v;
